...
(setq KEYWORDS_LIST '("and" "or" "not" "equal" "less" "nil" "list" "append" "concat" "set" "deffun" "for" "if" "exit" "load" "disp" "true" "false"))
	...
	(loop for ch across line do ;; ch is character ...
		...
		(cond 
			...
			( (not (eq (searchList (string ch) OPERATORS_LIST) nil))
				...
				(setq tokens (append tokens (list (nth index OPERATORS_TOKEN_LIST))))
			)
					...
					(progn 
						(if (not (equal (searchList word KEYWORDS_LIST) nil))
							(progn 
								...
								(setq tokens (append tokens (list (nth index KEYWORDS_TOKEN_LIST))))
								(setq word "")	
							)
			...
			(t (setq tokens (append tokens (list (return-error-message ch)))))
		)
	)
	(return-from determine-tokens-name tokens)
)
...
(defun gppinterpreter (filename)
	...
	(setf new-string-list (trim-list file_string))
	(helper-lexer new-string-list)
	(print-tokens TOKENS_LIST)
	
)

;; Calling gppinterpreter functions ...
(gppinterpreter "helloword.g++")